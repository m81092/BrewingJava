...
public class PropertyReaderUtilTest {
	...
	public void getPropertyValueTest() throws FileNotFoundException, IOException {
		...
		String result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "ALL_BOOKS");
		assertEquals("Test All books query", ALL_BOOKS, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "CATEGORY_BOOKS");
		assertEquals("Test Category books query", CATEGORY_BOOKS, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "FEATURED_BOOKS");
		assertEquals("Test Featured books query", FEATURED_BOOKS, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "BOOK_INFO");
		assertEquals("Test Book info query", BOOK_INFO, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "SAVE_USER_INFO");
		assertEquals("Test Save user info query", SAVE_USER_INFO, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "SAVE_ACCOUNT");
		assertEquals("Test Save account query", SAVE_ACCOUNT, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "LOGIN");
		assertEquals("Test Save Account", LOGIN, result);
		
		result = propertyReader.getPropertyValue(QUERIES_PROERTIES_FILE, "USER_INFO");
